...
Load community skills from `~/.agents/prompt-os/skills/community/` on demand:
- **superpowers** — shipping methodology (use for builds)
- **agent-skills** — production engineering (security, CI, observability)
- **grill-me** — requirements clarification (new capabilities)
- **anthropic-skills** — design & testing (UI builds)
- **ralph** — outer loop engine (long-running tasks)
- **app-store-review** — store submission audit (releases)
- **ccusage** — cost tracking (any task with LLM usage)

**Conflict resolution**: Only one methodology skill (superpowers) allowed. Load others on demand by task type.
